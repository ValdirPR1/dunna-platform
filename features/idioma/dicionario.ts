// Textos fixos do site público em português, inglês e espanhol.
// Cobre só a "casca" do site (menu, rodapé, textos de seção,
// formulário) — descrições de imóveis, empreendimentos e posts de
// blog continuam em português, porque vêm do banco de dados e
// exigiriam tradução de conteúdo, não de interface.
export type Idioma = "pt" | "en" | "es";

export const IDIOMAS: { codigo: Idioma; bandeira: string; label: string }[] = [
  { codigo: "pt", bandeira: "🇧🇷", label: "PT" },
  { codigo: "en", bandeira: "🇺🇸", label: "EN" },
  { codigo: "es", bandeira: "🇪🇸", label: "ES" },
];

export const dicionario = {
  pt: {
    navbar: {
      home: "Home",
      imoveis: "Imóveis",
      empreendimentos: "Empreendimentos",
      vender: "Vender meu imóvel",
      sobre: "Sobre",
      contato: "Contato",
      falarEspecialista: "Falar com especialista",
    },
    hero: {
      badge: "ESPECIALISTAS EM IMÓVEIS DE PRAIA",
      titulo1: "Viva o melhor do litoral.",
      titulo2: "Invista com segurança.",
      descricao:
        "Apartamentos, casas e empreendimentos selecionados em Porto de Galinhas, Muro Alto, Praia dos Carneiros, Tamandaré e São Miguel dos Milagres.",
      verImoveis: "Ver imóveis",
      empreendimentos: "Empreendimentos",
      anosMercado: "anos de mercado",
      regioesAtendidas: "regiões atendidas",
    },
    searchbar: {
      titulo: "Encontre seu imóvel ideal",
      regiao: "Região",
      tipo: "Tipo",
      quartos: "Quartos",
      valor: "Valor",
      buscar: "Buscar",
      regioes: {
        "porto-de-galinhas": "Porto de Galinhas",
        "muro-alto": "Muro Alto",
        "praia-dos-carneiros": "Praia dos Carneiros",
        tamandare: "Tamandaré",
        milagres: "Milagres",
      },
      tipos: {
        Apartamento: "Apartamento",
        Casa: "Casa",
        Studio: "Studio",
        Flat: "Flat",
      },
      opcoesQuartos: ["1 Quarto", "2 Quartos", "3 Quartos", "4+"],
      opcoesValor: [
        "Até R$300 mil",
        "R$300 a R$500 mil",
        "R$500 a R$800 mil",
        "Acima de R$800 mil",
      ],
    },
    cta: {
      titulo: "Encontre seu imóvel ideal.",
      descricao:
        "Nossa equipe está pronta para ajudar você a investir com segurança no litoral.",
      botao: "Falar com Especialista",
    },
    home: {
      tag: "IMÓVEIS EM DESTAQUE",
      titulo: "Oportunidades selecionadas",
      descricao:
        "Imóveis escolhidos pela equipe Dunna para morar, investir e rentabilizar.",
    },
    lancamentos: {
      tag: "EMPREENDIMENTOS",
      titulo: "Lançamentos e empreendimentos",
    },
    regioes: {
      tag: "DESTINOS",
      titulo: "Explore as melhores regiões",
      descricao:
        "Descubra os destinos mais desejados do litoral pernambucano e encontre o imóvel perfeito para investir ou aproveitar com sua família.",
      verImoveis: "Ver imóveis →",
    },
    blog: {
      tag: "BLOG",
      titulo: "Mercado imobiliário e novidades",
      verTodos: "Ver todos os artigos →",
    },
    avaliacoes: {
      tag: "AVALIAÇÕES DUNNA",
      titulo: "O que dizem sobre a gente",
      avaliacoesNoGoogle: "avaliações no Google",
      verTodas: "Ver todas as avaliações →",
      descricaoPagina:
        "Avaliações reais de quem já comprou, vendeu ou investiu em imóveis de praia com a Dunna, direto do nosso perfil no Google.",
      comBaseEm: "com base em",
      semAvaliacoes: "Nossas avaliações aparecerão aqui em breve.",
      deixarAvaliacao: "Deixar uma avaliação",
      verNoGoogle: "Ver no Google",
    },
    sobre: {
      heroTag: "SOBRE A DUNNA",
      heroTitulo:
        "Especialistas em imóveis de praia no litoral de Pernambuco.",
      heroDescricao:
        "Há mais de 10 anos conectamos pessoas aos melhores empreendimentos e imóveis do litoral pernambucano, unindo qualidade de vida, rentabilidade e segurança em cada negócio.",
      anosExperiencia: "anos de experiência",
      imoveisComercializados: "imóveis comercializados",
      regioesAtuacao: "regiões de atuação",
      porQueTag: "POR QUE A DUNNA",
      diferenciaisTitulo: "Nossos diferenciais",
      diferenciais: [
        {
          titulo: "Curadoria especializada",
          descricao:
            "Selecionamos apenas empreendimentos e imóveis com potencial real de valorização e rentabilidade.",
        },
        {
          titulo: "Acompanhamento completo",
          descricao:
            "Da primeira visita à assinatura do contrato, você tem um especialista dedicado ao seu lado.",
        },
        {
          titulo: "Conhecimento de região",
          descricao:
            "Vivemos e respiramos o litoral pernambucano — sabemos onde investir vale mais a pena.",
        },
      ],
      ondeAtuamosTag: "ONDE ATUAMOS",
      regioesTitulo: "Regiões de atuação",
      ctaTitulo: "Pronto para encontrar seu próximo imóvel?",
      ctaDescricao:
        "Fale com um de nossos especialistas e descubra as melhores oportunidades do litoral pernambucano.",
      ctaBotao: "Falar com a Dunna",
    },
    contato: {
      titulo: "Fale com a gente",
      descricao:
        "Preencha o formulário abaixo e um de nossos especialistas entra em contato para te ajudar a encontrar o imóvel ideal.",
      outrosCanais: "Outros canais",
      whatsapp: "WhatsApp",
      email: "E-mail",
      endereco: "Endereço",
      form: {
        nome: "Nome",
        nomePlaceholder: "Seu nome completo",
        telefone: "Telefone",
        telefonePlaceholder: "(00) 00000-0000",
        codigoPais: "Código do país",
        email: "E-mail",
        emailPlaceholder: "seu@email.com",
        mensagem: "Mensagem",
        mensagemPlaceholder: "Conte um pouco sobre o que você procura",
        enviar: "Enviar mensagem",
        enviando: "Enviando...",
        sucesso: "Mensagem enviada! Em breve entraremos em contato.",
        erro: "Não foi possível enviar agora. Tente novamente em instantes.",
      },
    },
    imoveis: {
      titulo: "Imóveis",
      descricao: "Encontre o imóvel ideal para morar, investir ou rentabilizar.",
      nenhumComFiltro: "Nenhum imóvel encontrado com esses filtros no momento.",
      nenhumSemFiltro: "Nenhum imóvel publicado no momento.",
    },
    imovelDetalhe: {
      voltar: "Voltar",
      secaoApresentacao: "Apresentação",
      secaoVideo: "Vídeo",
      secaoDetalhes: "Detalhes e Diferenciais",
      secaoLocalizacao: "Localização",
      codigo: "Código",
      dormitorios: "dormitórios",
      suites: "suítes",
      banheiros: "banheiros",
      vagas: "vagas",
      areaPrivativa: "de área privativa",
      sobreImovel: "Sobre o imóvel",
      descricaoEmBreve: "Descrição em breve.",
      valor: "Valor",
      condominio: "Condomínio",
      iptu: "IPTU",
      porAno: "/ano",
      porMes: "/mês",
      falarWhatsapp: "Falar via WhatsApp",
      compartilhar: "Compartilhar este imóvel",
      videoImovel: "Vídeo do imóvel",
      imoveisSemelhantes: "Imóveis Semelhantes",
      outrasOpcoes: "Outras opções com o mesmo perfil que podem te interessar.",
      equivalenteAproximado: "Equivalente aproximado",
    },
    agendarVisita: {
      botao: "Agendar Visita",
      imovel: "Imóvel",
      seuNome: "Seu nome",
      codigoPais: "Código do país",
      whatsappComDdd: "WhatsApp (com DDD)",
      manha: "Manhã",
      tarde: "Tarde",
      noite: "Noite",
      solicitar: "Solicitar Visita",
      enviando: "Enviando...",
      erroObrigatorio: "Preencha nome, telefone e a data preferida.",
      erroEnvio: "Não foi possível enviar. Tente novamente em instantes.",
      recebemos: "Recebemos sua solicitação!",
      corretorConfirma:
        "Um corretor vai confirmar o melhor horário com você em breve.",
      fechar: "Fechar",
    },
    whatsappModal: {
      titulo: "Fale com a gente",
      descricao: "Preenche rapidinho e te levamos direto pro WhatsApp.",
      seuNome: "Seu nome",
      emailOpcional: "E-mail (opcional)",
      codigoPais: "Código do país",
      whatsappComDdd: "WhatsApp (com DDD)",
      continuar: "Continuar no WhatsApp",
      enviando: "Enviando...",
      erroObrigatorio: "Preencha pelo menos nome e WhatsApp.",
      erroEnvio: "Não foi possível enviar. Tente novamente em instantes.",
    },
    vender: {
      tag: "VENDA COM A DUNNA",
      titulo: "Quer vender ou alugar seu imóvel?",
      descricao:
        "Conta pra gente sobre o seu imóvel e um corretor especialista entra em contato pra te ajudar a encontrar o comprador ideal.",
      beneficios: [
        {
          titulo: "Rede de compradores qualificados",
          texto: "Sua oferta chega direto pra quem já está buscando um imóvel na região.",
        },
        {
          titulo: "Avaliação justa de mercado",
          texto: "Analisamos a região e imóveis parecidos pra sugerir o melhor preço.",
        },
        {
          titulo: "Segurança em todo o processo",
          texto: "Da divulgação à assinatura do contrato, acompanhamos cada etapa.",
        },
        {
          titulo: "Resposta rápida",
          texto: "Um de nossos corretores entra em contato em até 24h úteis.",
        },
      ],
      formTitulo: "Conte sobre o seu imóvel",
      formSubtitulo: "Leva menos de 2 minutos.",
      form: {
        nome: "Seu nome",
        whatsapp: "WhatsApp (com DDD)",
        email: "E-mail (opcional)",
        cidade: "Cidade do imóvel",
        bairro: "Bairro (opcional)",
        tipoApartamento: "Apartamento",
        tipoCasa: "Casa",
        tipoTerreno: "Terreno",
        tipoComercial: "Comercial",
        tipoOutro: "Outro",
        quartos: "Quartos",
        valorPretendido: "Valor pretendido",
        observacoes: "Conte mais sobre o imóvel (opcional)",
        erroObrigatorio: "Preencha pelo menos nome, telefone e cidade do imóvel.",
        erroEnvio: "Não foi possível enviar. Tente novamente em instantes.",
        enviando: "Enviando...",
        enviar: "Quero vender meu imóvel",
        sucessoTitulo: "Recebemos suas informações!",
        sucessoTexto: "Um corretor especialista vai entrar em contato em breve.",
      },
    },
    empreendimentosPagina: {
      titulo: "Empreendimentos",
      nenhumPublicado: "Nenhum empreendimento publicado no momento.",
      secaoApresentacao: "Apresentação",
      secaoLocalizacao: "Localização",
      secaoLazer: "Lazer e Conveniência",
      secaoPlantas: "Tipologias e Plantas",
      secaoValorizacao: "Valorização",
      sobreEmpreendimento: "Sobre o empreendimento",
      descricaoEmBreve: "Descrição em breve.",
      fotosProjeto: "Fotos do Projeto",
      localizadoEm: "Localizado em",
      lazerConveniencia: "Lazer e Conveniência",
      tipologiasPlantas: "Tipologias e Plantas",
      potencialValorizacao: "Potencial de Valorização",
      falarEspecialista: "Falar com especialista",
      verUnidades: "Ver unidades disponíveis",
      compartilhar: "Compartilhar este empreendimento",
      mapaTitulo: "Mapa do empreendimento",
    },
    compartilhar: {
      copiarLink: "Copiar link",
      linkCopiado: "Link copiado!",
      whatsapp: "WhatsApp",
      email: "E-mail",
      instagram: "Instagram",
      funcionaCelular:
        "Esse botão funciona pelo celular. Copiei o link — é só colar no Instagram.",
      erroCompartilhar: "Não foi possível abrir o compartilhamento.",
    },
    galeria: {
      fechar: "Fechar",
      fotoAnterior: "Foto anterior",
      proximaFoto: "Próxima foto",
      maisFotos: "fotos",
      aPartirDe: "A partir de",
    },
    blogPagina: {
      tag: "BLOG DUNNA",
      titulo: "Mercado imobiliário e novidades",
      descricao:
        "Conteúdo sobre investimento, tendências e tudo que você precisa saber antes de comprar seu imóvel de praia.",
      semArtigos: "Em breve, novos artigos por aqui.",
      voltarBlog: "Voltar pro blog",
    },
    footer: {
      descricao:
        "Especialistas em imóveis de praia em Porto de Galinhas, Praia dos Carneiros e Tamandaré.",
      navegacao: "Navegação",
      contato: "Contato",
      direitosReservados: "Todos os direitos reservados.",
      links: {
        home: "Home",
        imoveis: "Imóveis",
        empreendimentos: "Empreendimentos",
        vender: "Vender meu imóvel",
        blog: "Blog",
        avaliacoes: "Avaliações",
        sobre: "Sobre",
        contato: "Contato",
      },
    },
  },
  en: {
    navbar: {
      home: "Home",
      imoveis: "Properties",
      empreendimentos: "Developments",
      vender: "Sell your property",
      sobre: "About",
      contato: "Contact",
      falarEspecialista: "Talk to a specialist",
    },
    hero: {
      badge: "BEACHFRONT PROPERTY SPECIALISTS",
      titulo1: "Live the best of the coast.",
      titulo2: "Invest with confidence.",
      descricao:
        "Apartments, houses and select developments in Porto de Galinhas, Muro Alto, Praia dos Carneiros, Tamandaré and São Miguel dos Milagres.",
      verImoveis: "View properties",
      empreendimentos: "Developments",
      anosMercado: "years in the market",
      regioesAtendidas: "regions covered",
    },
    searchbar: {
      titulo: "Find your ideal property",
      regiao: "Region",
      tipo: "Type",
      quartos: "Bedrooms",
      valor: "Price",
      buscar: "Search",
      regioes: {
        "porto-de-galinhas": "Porto de Galinhas",
        "muro-alto": "Muro Alto",
        "praia-dos-carneiros": "Praia dos Carneiros",
        tamandare: "Tamandaré",
        milagres: "Milagres",
      },
      tipos: {
        Apartamento: "Apartment",
        Casa: "House",
        Studio: "Studio",
        Flat: "Flat",
      },
      opcoesQuartos: ["1 Bedroom", "2 Bedrooms", "3 Bedrooms", "4+"],
      opcoesValor: [
        "Up to R$300k",
        "R$300k to R$500k",
        "R$500k to R$800k",
        "Above R$800k",
      ],
    },
    cta: {
      titulo: "Find your ideal property.",
      descricao:
        "Our team is ready to help you invest safely on the coast.",
      botao: "Talk to a Specialist",
    },
    home: {
      tag: "FEATURED PROPERTIES",
      titulo: "Selected opportunities",
      descricao:
        "Properties handpicked by the Dunna team to live in, invest, or generate rental income.",
    },
    lancamentos: {
      tag: "DEVELOPMENTS",
      titulo: "New launches and developments",
    },
    regioes: {
      tag: "DESTINATIONS",
      titulo: "Explore the best regions",
      descricao:
        "Discover the most sought-after destinations on the Pernambuco coast and find the perfect property to invest in or enjoy with your family.",
      verImoveis: "View properties →",
    },
    blog: {
      tag: "BLOG",
      titulo: "Real estate market news",
      verTodos: "View all articles →",
    },
    avaliacoes: {
      tag: "DUNNA REVIEWS",
      titulo: "What people are saying",
      avaliacoesNoGoogle: "reviews on Google",
      verTodas: "View all reviews →",
      descricaoPagina:
        "Real reviews from people who have bought, sold or invested in beachfront properties with Dunna, straight from our Google profile.",
      comBaseEm: "based on",
      semAvaliacoes: "Our reviews will appear here soon.",
      deixarAvaliacao: "Leave a review",
      verNoGoogle: "View on Google",
    },
    sobre: {
      heroTag: "ABOUT DUNNA",
      heroTitulo:
        "Beachfront property specialists on the coast of Pernambuco.",
      heroDescricao:
        "For over 10 years we've connected people with the best developments and properties on the Pernambuco coast, combining quality of life, profitability and security in every deal.",
      anosExperiencia: "years of experience",
      imoveisComercializados: "properties sold",
      regioesAtuacao: "regions covered",
      porQueTag: "WHY DUNNA",
      diferenciaisTitulo: "What sets us apart",
      diferenciais: [
        {
          titulo: "Specialized curation",
          descricao:
            "We select only developments and properties with real potential for appreciation and profitability.",
        },
        {
          titulo: "Full support",
          descricao:
            "From the first visit to signing the contract, you have a dedicated specialist by your side.",
        },
        {
          titulo: "Local knowledge",
          descricao:
            "We live and breathe the Pernambuco coast — we know where investing is truly worth it.",
        },
      ],
      ondeAtuamosTag: "WHERE WE OPERATE",
      regioesTitulo: "Regions we cover",
      ctaTitulo: "Ready to find your next property?",
      ctaDescricao:
        "Talk to one of our specialists and discover the best opportunities on the Pernambuco coast.",
      ctaBotao: "Talk to Dunna",
    },
    contato: {
      titulo: "Get in touch",
      descricao:
        "Fill out the form below and one of our specialists will reach out to help you find the ideal property.",
      outrosCanais: "Other channels",
      whatsapp: "WhatsApp",
      email: "Email",
      endereco: "Address",
      form: {
        nome: "Name",
        nomePlaceholder: "Your full name",
        telefone: "Phone",
        telefonePlaceholder: "(00) 00000-0000",
        codigoPais: "Country code",
        email: "Email",
        emailPlaceholder: "you@email.com",
        mensagem: "Message",
        mensagemPlaceholder: "Tell us a bit about what you're looking for",
        enviar: "Send message",
        enviando: "Sending...",
        sucesso: "Message sent! We'll be in touch soon.",
        erro: "Couldn't send right now. Please try again shortly.",
      },
    },
    imoveis: {
      titulo: "Properties",
      descricao: "Find the ideal property to live in, invest, or generate rental income.",
      nenhumComFiltro: "No properties found with these filters right now.",
      nenhumSemFiltro: "No properties published at the moment.",
    },
    imovelDetalhe: {
      voltar: "Back",
      secaoApresentacao: "Overview",
      secaoVideo: "Video",
      secaoDetalhes: "Details and Features",
      secaoLocalizacao: "Location",
      codigo: "Code",
      dormitorios: "bedrooms",
      suites: "en-suites",
      banheiros: "bathrooms",
      vagas: "parking spots",
      areaPrivativa: "of private area",
      sobreImovel: "About this property",
      descricaoEmBreve: "Description coming soon.",
      valor: "Price",
      condominio: "HOA fee",
      iptu: "Property tax",
      porAno: "/year",
      porMes: "/month",
      falarWhatsapp: "Chat on WhatsApp",
      compartilhar: "Share this property",
      videoImovel: "Property video",
      imoveisSemelhantes: "Similar Properties",
      outrasOpcoes: "Other options with the same profile that might interest you.",
      equivalenteAproximado: "Approximate equivalent",
    },
    agendarVisita: {
      botao: "Schedule a Visit",
      imovel: "Property",
      seuNome: "Your name",
      codigoPais: "Country code",
      whatsappComDdd: "WhatsApp number",
      manha: "Morning",
      tarde: "Afternoon",
      noite: "Evening",
      solicitar: "Request Visit",
      enviando: "Sending...",
      erroObrigatorio: "Fill in your name, phone and preferred date.",
      erroEnvio: "Couldn't send right now. Please try again shortly.",
      recebemos: "We've received your request!",
      corretorConfirma:
        "An agent will confirm the best time with you shortly.",
      fechar: "Close",
    },
    whatsappModal: {
      titulo: "Get in touch",
      descricao: "Fill this out quickly and we'll take you straight to WhatsApp.",
      seuNome: "Your name",
      emailOpcional: "Email (optional)",
      codigoPais: "Country code",
      whatsappComDdd: "WhatsApp number",
      continuar: "Continue on WhatsApp",
      enviando: "Sending...",
      erroObrigatorio: "Fill in at least name and WhatsApp number.",
      erroEnvio: "Couldn't send right now. Please try again shortly.",
    },
    vender: {
      tag: "SELL WITH DUNNA",
      titulo: "Want to sell or rent out your property?",
      descricao:
        "Tell us about your property and a specialist agent will reach out to help you find the ideal buyer.",
      beneficios: [
        {
          titulo: "Network of qualified buyers",
          texto: "Your listing reaches people who are already looking for a property in the region.",
        },
        {
          titulo: "Fair market valuation",
          texto: "We analyze the region and similar properties to suggest the best price.",
        },
        {
          titulo: "Security throughout the process",
          texto: "From marketing to signing the contract, we're with you every step of the way.",
        },
        {
          titulo: "Fast response",
          texto: "One of our agents will reach out within 24 business hours.",
        },
      ],
      formTitulo: "Tell us about your property",
      formSubtitulo: "Takes less than 2 minutes.",
      form: {
        nome: "Your name",
        whatsapp: "WhatsApp number",
        email: "Email (optional)",
        cidade: "Property city",
        bairro: "Neighborhood (optional)",
        tipoApartamento: "Apartment",
        tipoCasa: "House",
        tipoTerreno: "Land",
        tipoComercial: "Commercial",
        tipoOutro: "Other",
        quartos: "Bedrooms",
        valorPretendido: "Asking price",
        observacoes: "Tell us more about the property (optional)",
        erroObrigatorio: "Fill in at least name, phone and the property's city.",
        erroEnvio: "Couldn't send right now. Please try again shortly.",
        enviando: "Sending...",
        enviar: "I want to sell my property",
        sucessoTitulo: "We've received your information!",
        sucessoTexto: "A specialist agent will reach out shortly.",
      },
    },
    empreendimentosPagina: {
      titulo: "Developments",
      nenhumPublicado: "No developments published at the moment.",
      secaoApresentacao: "Overview",
      secaoLocalizacao: "Location",
      secaoLazer: "Leisure and Amenities",
      secaoPlantas: "Floor Plans",
      secaoValorizacao: "Appreciation",
      sobreEmpreendimento: "About this development",
      descricaoEmBreve: "Description coming soon.",
      fotosProjeto: "Project Photos",
      localizadoEm: "Located in",
      lazerConveniencia: "Leisure and Amenities",
      tipologiasPlantas: "Floor Plans",
      potencialValorizacao: "Appreciation Potential",
      falarEspecialista: "Talk to a specialist",
      verUnidades: "View available units",
      compartilhar: "Share this development",
      mapaTitulo: "Development map",
    },
    compartilhar: {
      copiarLink: "Copy link",
      linkCopiado: "Link copied!",
      whatsapp: "WhatsApp",
      email: "Email",
      instagram: "Instagram",
      funcionaCelular:
        "This button works on mobile. We've copied the link — just paste it on Instagram.",
      erroCompartilhar: "Couldn't open the share menu.",
    },
    galeria: {
      fechar: "Close",
      fotoAnterior: "Previous photo",
      proximaFoto: "Next photo",
      maisFotos: "photos",
      aPartirDe: "From",
    },
    blogPagina: {
      tag: "DUNNA BLOG",
      titulo: "Real estate market news",
      descricao:
        "Content about investing, trends and everything you need to know before buying your beachfront property.",
      semArtigos: "New articles coming soon.",
      voltarBlog: "Back to blog",
    },
    footer: {
      descricao:
        "Beachfront property specialists in Porto de Galinhas, Praia dos Carneiros and Tamandaré.",
      navegacao: "Navigation",
      contato: "Contact",
      direitosReservados: "All rights reserved.",
      links: {
        home: "Home",
        imoveis: "Properties",
        empreendimentos: "Developments",
        vender: "Sell your property",
        blog: "Blog",
        avaliacoes: "Reviews",
        sobre: "About",
        contato: "Contact",
      },
    },
  },
  es: {
    navbar: {
      home: "Inicio",
      imoveis: "Propiedades",
      empreendimentos: "Emprendimientos",
      vender: "Vender mi propiedad",
      sobre: "Nosotros",
      contato: "Contacto",
      falarEspecialista: "Hablar con un especialista",
    },
    hero: {
      badge: "ESPECIALISTAS EN PROPIEDADES DE PLAYA",
      titulo1: "Vive lo mejor del litoral.",
      titulo2: "Invierte con seguridad.",
      descricao:
        "Apartamentos, casas y emprendimientos seleccionados en Porto de Galinhas, Muro Alto, Praia dos Carneiros, Tamandaré y São Miguel dos Milagres.",
      verImoveis: "Ver propiedades",
      empreendimentos: "Emprendimientos",
      anosMercado: "años en el mercado",
      regioesAtendidas: "regiones atendidas",
    },
    searchbar: {
      titulo: "Encuentra tu propiedad ideal",
      regiao: "Región",
      tipo: "Tipo",
      quartos: "Habitaciones",
      valor: "Precio",
      buscar: "Buscar",
      regioes: {
        "porto-de-galinhas": "Porto de Galinhas",
        "muro-alto": "Muro Alto",
        "praia-dos-carneiros": "Praia dos Carneiros",
        tamandare: "Tamandaré",
        milagres: "Milagres",
      },
      tipos: {
        Apartamento: "Apartamento",
        Casa: "Casa",
        Studio: "Studio",
        Flat: "Flat",
      },
      opcoesQuartos: ["1 Habitación", "2 Habitaciones", "3 Habitaciones", "4+"],
      opcoesValor: [
        "Hasta R$300 mil",
        "R$300 a R$500 mil",
        "R$500 a R$800 mil",
        "Más de R$800 mil",
      ],
    },
    cta: {
      titulo: "Encuentra tu propiedad ideal.",
      descricao:
        "Nuestro equipo está listo para ayudarte a invertir con seguridad en el litoral.",
      botao: "Hablar con un Especialista",
    },
    home: {
      tag: "PROPIEDADES DESTACADAS",
      titulo: "Oportunidades seleccionadas",
      descricao:
        "Propiedades elegidas por el equipo Dunna para vivir, invertir o rentabilizar.",
    },
    lancamentos: {
      tag: "EMPRENDIMIENTOS",
      titulo: "Nuevos lanzamientos y emprendimientos",
    },
    regioes: {
      tag: "DESTINOS",
      titulo: "Explora las mejores regiones",
      descricao:
        "Descubre los destinos más deseados del litoral de Pernambuco y encuentra la propiedad perfecta para invertir o disfrutar con tu familia.",
      verImoveis: "Ver propiedades →",
    },
    blog: {
      tag: "BLOG",
      titulo: "Mercado inmobiliario y novedades",
      verTodos: "Ver todos los artículos →",
    },
    avaliacoes: {
      tag: "RESEÑAS DUNNA",
      titulo: "Lo que dicen sobre nosotros",
      avaliacoesNoGoogle: "reseñas en Google",
      verTodas: "Ver todas las reseñas →",
      descricaoPagina:
        "Reseñas reales de quienes ya compraron, vendieron o invirtieron en propiedades de playa con Dunna, directo desde nuestro perfil de Google.",
      comBaseEm: "con base en",
      semAvaliacoes: "Nuestras reseñas aparecerán aquí pronto.",
      deixarAvaliacao: "Dejar una reseña",
      verNoGoogle: "Ver en Google",
    },
    sobre: {
      heroTag: "SOBRE DUNNA",
      heroTitulo:
        "Especialistas en propiedades de playa en el litoral de Pernambuco.",
      heroDescricao:
        "Hace más de 10 años conectamos personas con los mejores emprendimientos y propiedades del litoral de Pernambuco, uniendo calidad de vida, rentabilidad y seguridad en cada negocio.",
      anosExperiencia: "años de experiencia",
      imoveisComercializados: "propiedades comercializadas",
      regioesAtuacao: "regiones de actuación",
      porQueTag: "POR QUÉ DUNNA",
      diferenciaisTitulo: "Nuestros diferenciales",
      diferenciais: [
        {
          titulo: "Curaduría especializada",
          descricao:
            "Seleccionamos solo emprendimientos y propiedades con potencial real de valorización y rentabilidad.",
        },
        {
          titulo: "Acompañamiento completo",
          descricao:
            "Desde la primera visita hasta la firma del contrato, tienes un especialista dedicado a tu lado.",
        },
        {
          titulo: "Conocimiento de la región",
          descricao:
            "Vivimos y respiramos el litoral de Pernambuco — sabemos dónde invertir realmente vale la pena.",
        },
      ],
      ondeAtuamosTag: "DÓNDE ACTUAMOS",
      regioesTitulo: "Regiones de actuación",
      ctaTitulo: "¿Listo para encontrar tu próxima propiedad?",
      ctaDescricao:
        "Habla con uno de nuestros especialistas y descubre las mejores oportunidades del litoral de Pernambuco.",
      ctaBotao: "Hablar con Dunna",
    },
    contato: {
      titulo: "Habla con nosotros",
      descricao:
        "Completa el formulario a continuación y uno de nuestros especialistas se pondrá en contacto para ayudarte a encontrar la propiedad ideal.",
      outrosCanais: "Otros canales",
      whatsapp: "WhatsApp",
      email: "Correo electrónico",
      endereco: "Dirección",
      form: {
        nome: "Nombre",
        nomePlaceholder: "Tu nombre completo",
        telefone: "Teléfono",
        telefonePlaceholder: "(00) 00000-0000",
        codigoPais: "Código de país",
        email: "Correo electrónico",
        emailPlaceholder: "tu@email.com",
        mensagem: "Mensaje",
        mensagemPlaceholder: "Cuéntanos un poco sobre lo que buscas",
        enviar: "Enviar mensaje",
        enviando: "Enviando...",
        sucesso: "¡Mensaje enviado! Pronto nos pondremos en contacto.",
        erro: "No fue posible enviar ahora. Intenta de nuevo en un momento.",
      },
    },
    imoveis: {
      titulo: "Propiedades",
      descricao: "Encuentra la propiedad ideal para vivir, invertir o rentabilizar.",
      nenhumComFiltro: "No se encontraron propiedades con esos filtros por el momento.",
      nenhumSemFiltro: "No hay propiedades publicadas por el momento.",
    },
    imovelDetalhe: {
      voltar: "Volver",
      secaoApresentacao: "Presentación",
      secaoVideo: "Video",
      secaoDetalhes: "Detalles y Diferenciales",
      secaoLocalizacao: "Ubicación",
      codigo: "Código",
      dormitorios: "habitaciones",
      suites: "suites",
      banheiros: "baños",
      vagas: "plazas de garaje",
      areaPrivativa: "de área privativa",
      sobreImovel: "Sobre la propiedad",
      descricaoEmBreve: "Descripción próximamente.",
      valor: "Precio",
      condominio: "Condominio",
      iptu: "Impuesto predial",
      porAno: "/año",
      porMes: "/mes",
      falarWhatsapp: "Hablar por WhatsApp",
      compartilhar: "Compartir esta propiedad",
      videoImovel: "Video de la propiedad",
      imoveisSemelhantes: "Propiedades Similares",
      outrasOpcoes: "Otras opciones con el mismo perfil que pueden interesarte.",
      equivalenteAproximado: "Equivalente aproximado",
    },
    agendarVisita: {
      botao: "Agendar Visita",
      imovel: "Propiedad",
      seuNome: "Tu nombre",
      codigoPais: "Código de país",
      whatsappComDdd: "WhatsApp (con código de área)",
      manha: "Mañana",
      tarde: "Tarde",
      noite: "Noche",
      solicitar: "Solicitar Visita",
      enviando: "Enviando...",
      erroObrigatorio: "Completa nombre, teléfono y la fecha preferida.",
      erroEnvio: "No fue posible enviar. Intenta de nuevo en un momento.",
      recebemos: "¡Recibimos tu solicitud!",
      corretorConfirma:
        "Un agente confirmará el mejor horario contigo pronto.",
      fechar: "Cerrar",
    },
    whatsappModal: {
      titulo: "Habla con nosotros",
      descricao: "Completa esto rápido y te llevamos directo a WhatsApp.",
      seuNome: "Tu nombre",
      emailOpcional: "Correo electrónico (opcional)",
      codigoPais: "Código de país",
      whatsappComDdd: "WhatsApp (con código de área)",
      continuar: "Continuar en WhatsApp",
      enviando: "Enviando...",
      erroObrigatorio: "Completa al menos nombre y WhatsApp.",
      erroEnvio: "No fue posible enviar. Intenta de nuevo en un momento.",
    },
    vender: {
      tag: "VENDE CON DUNNA",
      titulo: "¿Quieres vender o alquilar tu propiedad?",
      descricao:
        "Cuéntanos sobre tu propiedad y un agente especialista se pondrá en contacto para ayudarte a encontrar el comprador ideal.",
      beneficios: [
        {
          titulo: "Red de compradores calificados",
          texto: "Tu oferta llega directo a quienes ya están buscando una propiedad en la región.",
        },
        {
          titulo: "Valoración justa de mercado",
          texto: "Analizamos la región y propiedades similares para sugerir el mejor precio.",
        },
        {
          titulo: "Seguridad en todo el proceso",
          texto: "Desde la divulgación hasta la firma del contrato, acompañamos cada etapa.",
        },
        {
          titulo: "Respuesta rápida",
          texto: "Uno de nuestros agentes se pondrá en contacto en hasta 24h hábiles.",
        },
      ],
      formTitulo: "Cuéntanos sobre tu propiedad",
      formSubtitulo: "Toma menos de 2 minutos.",
      form: {
        nome: "Tu nombre",
        whatsapp: "WhatsApp (con código de área)",
        email: "Correo electrónico (opcional)",
        cidade: "Ciudad de la propiedad",
        bairro: "Barrio (opcional)",
        tipoApartamento: "Apartamento",
        tipoCasa: "Casa",
        tipoTerreno: "Terreno",
        tipoComercial: "Comercial",
        tipoOutro: "Otro",
        quartos: "Habitaciones",
        valorPretendido: "Precio deseado",
        observacoes: "Cuéntanos más sobre la propiedad (opcional)",
        erroObrigatorio: "Completa al menos nombre, teléfono y ciudad de la propiedad.",
        erroEnvio: "No fue posible enviar. Intenta de nuevo en un momento.",
        enviando: "Enviando...",
        enviar: "Quiero vender mi propiedad",
        sucessoTitulo: "¡Recibimos tu información!",
        sucessoTexto: "Un agente especialista se pondrá en contacto pronto.",
      },
    },
    empreendimentosPagina: {
      titulo: "Emprendimientos",
      nenhumPublicado: "Ningún emprendimiento publicado por el momento.",
      secaoApresentacao: "Presentación",
      secaoLocalizacao: "Ubicación",
      secaoLazer: "Ocio y Comodidades",
      secaoPlantas: "Tipologías y Planos",
      secaoValorizacao: "Valorización",
      sobreEmpreendimento: "Sobre el emprendimiento",
      descricaoEmBreve: "Descripción próximamente.",
      fotosProjeto: "Fotos del Proyecto",
      localizadoEm: "Ubicado en",
      lazerConveniencia: "Ocio y Comodidades",
      tipologiasPlantas: "Tipologías y Planos",
      potencialValorizacao: "Potencial de Valorización",
      falarEspecialista: "Hablar con un especialista",
      verUnidades: "Ver unidades disponibles",
      compartilhar: "Compartir este emprendimiento",
      mapaTitulo: "Mapa del emprendimiento",
    },
    compartilhar: {
      copiarLink: "Copiar enlace",
      linkCopiado: "¡Enlace copiado!",
      whatsapp: "WhatsApp",
      email: "Correo electrónico",
      instagram: "Instagram",
      funcionaCelular:
        "Este botón funciona en el celular. Copiamos el enlace — solo pégalo en Instagram.",
      erroCompartilhar: "No fue posible abrir el menú para compartir.",
    },
    galeria: {
      fechar: "Cerrar",
      fotoAnterior: "Foto anterior",
      proximaFoto: "Foto siguiente",
      maisFotos: "fotos",
      aPartirDe: "Desde",
    },
    blogPagina: {
      tag: "BLOG DUNNA",
      titulo: "Mercado inmobiliario y novedades",
      descricao:
        "Contenido sobre inversión, tendencias y todo lo que necesitas saber antes de comprar tu propiedad de playa.",
      semArtigos: "Pronto, nuevos artículos por aquí.",
      voltarBlog: "Volver al blog",
    },
    footer: {
      descricao:
        "Especialistas en propiedades de playa en Porto de Galinhas, Praia dos Carneiros y Tamandaré.",
      navegacao: "Navegación",
      contato: "Contacto",
      direitosReservados: "Todos los derechos reservados.",
      links: {
        home: "Inicio",
        imoveis: "Propiedades",
        empreendimentos: "Emprendimientos",
        vender: "Vender mi propiedad",
        blog: "Blog",
        avaliacoes: "Reseñas",
        sobre: "Nosotros",
        contato: "Contacto",
      },
    },
  },
};
